...
void printPrivateValues(Function& F, ostream& OS){
  unsigned prev = 0;
  bool endLabel = false;
  for (BasicBlock &B : F){
    for (Instruction &I : B){
...
bool ProgramDependencyGraph::runOnModule(Module &M)
{
  ...
  while(!queue.empty()){
    ...
    std::set<string> privateValuesOnInstW;
    bool isLoopBranch = false;

    //find private value set on the current node
    if(privateMap.find(InstW->getInstruction()) != privateMap.end()){
      privateValuesOnInstW = privateMap[InstW->getInstruction()];

      //print privateValuesOnInstW
      errs() << InstW->getInstruction()->getDebugLoc().getLine()<<" privateValuesOnInstW: ";
      for(auto const &c: privateValuesOnInstW){
	errs() << c << " ";
      }
	...
	if(false == adjacentInstW->getFlag()){
	  ...
	  if(isa<StoreInst>(I)){
	    ...
	    errs() << "privateMap[I]: " ;
	    for(auto const& c : privateMap.find(I)->second){
	      errs() << c << " " ;
	    }
	    errs() << "\n";
	  }
  ...
  /*
    ...
    if(privateMap.find(I) != privateMap.end()){
    //	errs() << privateMap.find(I)->second.size() << "\n";
    for(auto & pv : privateMap.find(I)->second){
    errs() << pv << " ";
    }
    errs()<< "\n";	
    }
